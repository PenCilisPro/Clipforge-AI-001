...
const STATUS_DETAILS: Partial<Record<ProcessingStatus, string>> = {
  ...
  RENDERING: 'Rendering is active. The percentage below comes from render_jobs.',
...
export function ProcessingProgressTracker({
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => void loadRenderProgress(), 1500)
    return () => {
      cancelled = true
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-zinc-400">{STATUS_DETAILS[status] ?? 'Waiting for the backend processing pipeline...'}</p>
            </div>
          </div>
