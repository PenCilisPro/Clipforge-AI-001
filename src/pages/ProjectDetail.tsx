...
import type { Project, Video, Transcript, Clip, ProcessingStatus } from '@/lib/types'
...
const STAGE_MESSAGES: Partial<Record<ProcessingStatus, string>> = {
  ...
  MATCHING_PATTERNS: 'Matching your clipping patterns…',
  FINDING_CLIPS: 'Finding high-retention moments…',
  GENERATING_CONFIG: 'Creating your clips…',
  RENDERING: 'Rendering with Remotion…',
  ADDING_CAPTIONS: 'Adding captions…',
  FINDING_BROLL: 'Finding relevant B-roll…',
  ADDING_MUSIC: 'Adding music…',
  UPLOADING_RENDER: 'Uploading final render…',
...
export default function ProjectDetail() {
  ...
  // Poll while the project is processing so the page stays live.
  useEffect(() => {
    if (!project || ['COMPLETED', 'FAILED'].includes(project.status)) return
    const interval = setInterval(() => void loadAll(), 4000)
  ...
  async function retryProcessing() {
    ...
    await supabase
      ...
      .eq('id', projectId)
    await invokeFunction('process-video', { projectId }).catch(() => {})
    await loadAll()
    setRetrying(false)
  }
  ...
  return (
    <div>
      ...
      {isProcessing && (
        <div className="card mb-6 p-5">
          <p className="mb-3 text-sm font-medium text-brand-400">
            {STAGE_MESSAGES[project.status] ?? 'Processing…'}
          </p>
      ...
      {project.status === 'FAILED' && (
        <div className="mb-6">
          <ErrorState
            message={project.error_message ?? 'Processing failed.'}
            onRetry={retrying ? undefined : () => void retryProcessing()}
          />
        </div>
      )}
