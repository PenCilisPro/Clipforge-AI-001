...
function makeFilters(config:ClipConfiguration,broll:Array<BrollConfigItem & {file:string}>,music:boolean,voice:boolean,w:number,h:number){
  ...
  let audio='0:a:0'; if(music||voice){const inputs=['[0:a]'];if(music)inputs.push(`[${broll.length+1}:a]`);if(voice)inputs.push(`[${broll.length+1+(music?1:0)}:a]`);filters.push(`${inputs.join('')}amix=inputs=${inputs.length}:duration=first:dropout_transition=2[aout]`);audio='[aout]'} return {graph:filters.join(';'),video,audio}
}
