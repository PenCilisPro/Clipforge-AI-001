...
async function getSource(projectId: string, sourceType: string|null, sourceUrl: string|null, work: string) {
  const out=path.join(work,'source.mp4'); const {data}=await supabase.from('videos').select('storage_path').eq('project_id',projectId).maybeSingle(); const storage=typeof data?.storage_path==='string'?data.storage_path.trim():''
